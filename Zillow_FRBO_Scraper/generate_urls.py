...
# URL 1: Max Payment $1800
state1 = copy.deepcopy(state)
state1['filterState']['mp'] = {'max': 1800}
# Ensure price max is consistent or removed to avoid conflicts
# For rentals, 'price' usually mirrors 'mp' but let's just leave it high/default from original
url1_state = json.dumps(state1)
url1 = f"https://www.zillow.com/chicago-il/rentals/?searchQueryState={urllib.parse.quote(url1_state)}"

# URL 2: Min Payment $1801, Max 300000
state2 = copy.deepcopy(state)
state2['filterState']['mp'] = {'min': 1801}
# Ensure the max price is set as requested
state2['filterState']['price'] = {'max': 300000} 
url2_state = json.dumps(state2)
url2 = f"https://www.zillow.com/chicago-il/rentals/?searchQueryState={urllib.parse.quote(url2_state)}"
